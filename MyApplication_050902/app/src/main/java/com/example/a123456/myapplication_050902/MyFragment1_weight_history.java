...
public class MyFragment1_weight_history extends AppCompatActivity {
    private Queue<String> Userweight = new LinkedList<String>();//用于存储数据库的数据，然后作为历史记录的数据
    private Queue<String> Usertime = new LinkedList<String>();
    private Queue<String> Userweight2 = new LinkedList<String>();//作为折线图的数据。
...
//        setContentView(R.layout.fragment1_weight_history);
//
//    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.i("weight_history", "————当前用户为————————：" + User_id);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    String sql = "select * from userweight where Userid ='" + User_id + "'order by Testtime desc;";
                    ...
                    while (rs.next()) {
                        ...
                        //Log.i("weight_history_BMI","BMI:"+UserBMI.poll());
                        //Log.i("weight_history_weight1","weight:"+Userweight.poll()+"\t time:"+Usertime.poll());
                        //Log.i("weight_history_weight2","weight2:"+Userweight2.poll()+"\t time2"+Usertime2.poll());
    ...
    /*
     *
     * 接收到myHandler发送的消息：0x127后开始做折线图和历史数据。
     */
    final Handler myHandler = new Handler() {//这里更改掉了旧方法，使得handler里面的内容可以真正的实现“数据库读取完成后再开始更改UI”。
        public void handleMessage(Message msg) {
            if (msg.what == 0x127) {
                ...
                SimpleAdapter myAdapter2 = new SimpleAdapter(getApplicationContext(), listitem2, R.layout.fragment1_heart_adapter2, new String[]{"tizhong"}, new int[]{R.id.heart_heart});
                ...
                ListView listView = (ListView) findViewById(R.id.list_weight_view);
                ListView listView2 = (ListView) findViewById(R.id.list_weight_view2);
                ListView listView3 = (ListView) findViewById(R.id.list_weight_view3);

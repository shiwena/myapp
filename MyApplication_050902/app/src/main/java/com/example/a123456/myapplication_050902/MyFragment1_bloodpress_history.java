...
public class MyFragment1_bloodpress_history extends AppCompatActivity {
    ...
    private Queue<String> Usertime3 = new LinkedList<String>();
            ...
            public void run() {
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        Usertime3.offer(t2.format(date3));
...
//	    data.setAxisXTop(axisX);  //x 轴在顶部
        ...
        Axis axisY = new Axis();  //Y轴
        axisY.setName("舒张压(单位：mmhg)");//y轴标注
        axisY.setTextColor(Color.parseColor("#666666"));
        axisY.setTextSize(11);//设置字体大小
        data.setAxisYLeft(axisY);  //Y轴设置在左边
        //data.setAxisYRight(axisY);  //y轴设置在右边
        ...
        lineChart2.setMaxZoom((float) 5);//缩放比例,比例越大，密集程度就越低
    ...
    private void getAxisXLables2() {
        ...
        while (Usertime3.peek() != null) {
            mAxisXValues2.add(new AxisValue(i).setLabel(Usertime3.poll()));
            i++;
        }
    ...
    private void getAxisPoints2() {
        ...
        while (Usershuzhangya2.peek() != null) {
            mPointValues2.add(new PointValue(i, Float.parseFloat(Usershuzhangya2.poll())));
            i++;
        }
